...
enum Variables {
  SettingsClass = "Mf",
  OtherSettingClass = "bo",
  SettingEnum = "P.A",
  KeybindEnum = "ge.A",
  SettingUIFunction = "po",
}
...
class PolyModLoaderImpl implements PolyModLoader {
  ...
  #settingElements: Array<HTMLElement> = [];
  ...
  #settingIndex = 0;
  addCustomClickableButtons(settingText: string, buttons: {text: string, callback: Function}[]) {
    let a = document.createElement("div");
    if (((a.className = "setting"), null != settingText)) {
      const t = document.createElement("p");
      ((t.textContent = settingText), a.appendChild(t));
    } else a.classList.add("wrappable");
    ...
    this.#appendToSettingsMenu(a);
  }
  addToSettings(index: number) {
    this.getFromPolyTrack(`(0, R.gn)(ActivePolyModLoader.otherSettingClass, to, "f")`).appendChild(this.#settingElements[index]);
  }
  #appendToSettingsMenu(element: HTMLElement) {
  ...
  registerSetting(name: string, id: string, type: SettingType, defaultOption: any, optionsOptional?: Array<{ title: string, value: string }>) {
    ...
    if (type === "boolean") {
      ...
      this.#settings.push(`(0, R.gn)(this, qs, "m", vo).call(
              this,
              "${name}",
              [
                {
      ...
      this.#settings.push(`(0, R.gn)(this, qs, "m", Ao).call(
              this,
              "${name}",
              ${Variables.SettingEnum}.${id},
            ),`)
  ...
  registerKeybind(name: string, id: string, event: string, defaultBind: string, secondBindOptional: string | null, callback: Function) {
    this.#latestBinding++;
    this.#keybindings.push(`(0, R.gn)(this, qs, "m", yo).call(
              this,
              "${name}",
      ...
      switch (mixinType) {
        case MixinType.INSERT:
          ...
          if (tokenIndex === -1) {
            console.log(tokenIndex);
            throw new Error(
              `Token "${token}" not found in "${error}".`
            );
          }
        ...
        case MixinType.REMOVEBETWEEN:
          ...
          if (firstTokenIndex === -1) {
            throw new Error(
              `Token "${tokenStart}" not found in "${error}".`
            );
          }
          if (secondTokenIndex === -1) {
            throw new Error(
              `Token "${tokenEnd}" not found in "${error}".`
            );
          }
